...
\begin{table}[H]
	...
	\hline
	IED & Initial Exchange Date & Date of first principal event, start of accrual calculation & 1 \\
	\hline
	IPCI & Interest Capitalization & Periodic interest payment, however capitalized & 2 \\
	\hline
	IP & Interest Payment & Periodic interest payment & 3 \\
	\hline
	FP & Fee Payment & (Periodic) fee payment & 4 \\
	\hline
	PR & Principal Redemption & (Periodic) principal redemption payment & 5 \\
	\hline
	PY & Penalty Payment & Payment of a penalty (e.g. due to early repayment of principal outstanding) & 6 \\
	\hline
	PP & Principal Prepayment & Early repayment of principal outstanding & 7 \\
	\hline
	CD & Credit Default & Indicates whether a contract is in its default state or not & 8 \\
	\hline
	RRY & Next Rate Reset & Rate reset event where new rate is already known & 9 \\
	\hline
	RR & Rate Reset & Interest rate is reset periodically & 10 \\
	\hline
	DV & Dividend Payment & Periodic dividend payment & 11 \\
	\hline
	PRD & Purchase Date & Purchase date of a contract bought in the secondary market & 12 \\
	\hline
	MR & Margin Call Date & Margin call event & 13 \\
	\hline
	TD & Termination Date & Sell date of a contract sold in the secondary market & 14 \\
	\hline
	SC & Scaling Index Revision & & 15 \\
	\hline
	IPCB & Interest Payment Calculation Base & Updates the calculation base for IP accruing & 16 \\
	\hline
	XD & Expiration Date & Marks expiration of e.g. an OPTNS or FUTUR contract & 17 \\
	\hline
	STD & Settlement Date & Date when payment for derivatives is settled & 18 \\
	\hline
	AD & Analysis Event & Retrieves current contract states without alter these & 19 \\
	\hline
	\end{tabular}
...
\begin{schedule}{OPTNS}
	...
	XD & $t^{XD} = \attr{MD}$ &  \\
	\hline
	STD & $t^{XD} = \begin{cases} \attr{MD} & \text{if}\quad \attr{STD}=\undef \\
					\attr{STD} & \text{else} \end{cases}$ &  \\
	\hline
	CD & & Same as PAM \\
\end{schedule}
...
\begin{states}{OPTNS}
	$\svar{Pos}{}$ & $\svar{Pos}{t_0} = \begin{cases} 
			...
			$S_{t_0}=\obs{rf}{\cldca{Child}{MOC}}{t_0}$ \\
	\hline	
	$\svar{Prf}{}$ & & Same as PAM \\
	\hline
	$\svar{Led}{}$ & & Same as PAM \\
...
\begin{functions}{OPTNS}
	AD & \pof{AD}{PAM} & {$\begin{aligned} 
					...
					\svar{Led}{t^+} &= t \end{aligned}$}\par
with\par
			$S_t=\obs{rf}{\cldca{Child}{MOC}}{t}$ \\
	\hline
	PRD & $\dfl{t^-}(-1)\attr{PPRD}$ & $\svar{Led}{t^+} = t$ \\
	\hline
	TD & $\dfl{t^-}\attr{PTD}$ & $\svar{Led}{t^+} = t$ \\
	\hline
...
with\par
			$S_t=\obs{rf}{\cldca{Child}{MOC}}{t}$ \\
	\hline
	PRD & \pof{PRD}{OPTNS} & \stf{PRD}{OPTNS} \\
	\hline
	TD & \pof{TD}{OPTNS} & \stf{TD}{OPTNS} \\
	\hline
	MD & \pof{MD}{OPTNS} & {$\begin{aligned} 
					\svar{Pos}{t^+} &= S_t-\attr{PFUT} \\
					\svar{Led}{t^+} &= t \end{aligned}$}\par
with\par
			$S_t=\obs{rf}{\cldca{Child}{MOC}}{t}$ \\
