...
\begin{functions}{PAM}
	...
	IED & $\obs{rf}{\attr{CURS}}{t}\sgn(-1)(\attr{NT}+\attr{PDIED})$
		...
		$y=\yfr{\attr{IPANX}}{t}$ \\
	\hline
	MD & $\obs{rf}{\attr{CURS}}{t}(\svar{Nsc}{t^-}\svar{Nt}{t^-}+\svar{Isc}{t^-}\svar{Ipac}{t^-}+\svar{Feac}{t^-})$
		& {$\begin{aligned}
			\svar{Nt}{t^+} &= 0.0 \\
			\svar{Ipac}{t^+} &= 0.0 \\
			\svar{Feac}{t^+} &= 0.0 \\
	...
	\hline
	PP & $\obs{rf}{\attr{CURS}}{t}\fev{\obs{ev}{\attr{CID},\attr{PP}}{t}}$
		& {$\begin{aligned}
...
\begin{functions}{FXOUT}
	...
	\hline
	STD & $\obs{rf}{\attr{CURS}}{t}\sgn (\attr{NT}-\obs{rf}{i}{\svar{Md}{t}}\attr{NT2})$\par
		where\par
...
with\par
	...
	\hline
	STD & $\obs{rf}{\attr{CURS}}{t}\sgn\svar{Xa}{t^-}$ & {$\begin{aligned}
					\svar{Xa}{t^+} &= 0.0 \\
					\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
...
with\par
	...
	FP & {$\begin{aligned} &\obs{rf}{\attr{CURS}}{t}\sgn\attr{FER} & \text{if} \quad \attr{FEB}=A \\
				&\obs{rf}{\attr{CURS}}{t}\left(\svar{Feac}{t^-} + \svar{Nt}{t^-}\yfr{t^-}{t}\attr{FER}\right) & \text{else} \end{aligned}$}
			& {$\begin{aligned}
					\svar{Nt}{t^+} &= \begin{cases}
							\svar{Nt}{t^-} & \text{if}\quad \attr{NT}\neq\undef \\
							\attr{CECV}\sum_{i=1}^{\mid I\mid} n_i & \text{else} \end{cases} \\
...
with\par
	...
	$I=\attr{CTST}_{CoveredContract}^{Contract}$ \\
	\hline
	STD & $\obs{rf}{\attr{CURS}}{t}\left(\svar{Xa}{t^-}+\svar{Feac}{t^-}\right)$ & {$\begin{aligned}
					\svar{Nt}{t^+} &= 0.0 \\
					\svar{Feac}{t^+} &= 0.0 \\
					\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
