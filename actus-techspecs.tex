...
\begin{table}[H]
  ...
  ??? & 1.1 & Streamlind with dictionary updates
	\begin{itemize}
		...
		\item Added \attr{ContractStructure}
...
Performance of a contract indicates whether as per a certain time all parties involved adhere to their obligations arising from the contract. Attribute \attr{PRF} captures a contract's performance as per $t_0$. For any time $t>t_0$ and depending on the behavior of the parties involved with the contract can migrate into different performance states from \textit{'performing'} to \textit{'default'}. State variable $\svar{Prf}{t}$ (cf. table \ref{tbl:statevars}) captures these dynamics and the performance as per any time $t>t_0$.\\

The Contract Default Convention is a function $D$ that maps the $\svar{Prf}{t}$ state variable into $+1$ indicating that the contract is performing or $0$ which reflects default and, from an analytical perspective, means that future cash flows \textit{cancel out}:
...
\begin{functions}{PAM}
	...
	PY & {$\begin{aligned}
		...
		with\par
		$c=\dfl{t^-}\sgn\yfr{\svar{Sd}{t^-}}{t}\svar{Nt}{t^-}$
		& {$\begin{aligned}
			\svar{Ipac}{t^+} &= \svar{Ipac}{t^-} + \yfr{\svar{Sd}{t^-}}{t}\svar{Ipnr}{t^-}\svar{Nt}{t^-} \\
			\svar{Fac}{t^+} &= \begin{cases} \svar{Fac}{t^-} + \yfr{\svar{Sd}{t^-}}{t}\svar{Nt}{t^-}\attr{FER} & \text{if} \quad \attr{FEB}=\text{'N'} \\
					\frac{\yfr{t^{FP-}}{t}}{\yfr{t^{FP-}}{t^{FP+}}}\attr{FER} & \text{else} \end{cases} \\
	...
	FP & {$\begin{aligned}
		...
		& {$\begin{aligned}
			...
			\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
  	PRD & $\dfl{t^-}\sgn (-1)(\attr{PPRD} + \svar{Ipac}{t^-} +$ \par
		...
		{$\begin{aligned}
			...
			t^{FP+} &= \inf t \in \vec{t}^{FP}\mid t>t_0 \end{aligned}$} \\
	\hline
  	TD & $\dfl{t^-}\sgn (\attr{PTD} + \svar{Ipac}{t^-} +$ \par
		$\qquad\qquad \yfr{\svar{Sd}{t^-}}{t}\svar{Ipnr}{t^-}\svar{Nt}{t^-})$
		& {$\begin{aligned}
			\svar{Nt}{t^+} &= 0.0 \\
			\svar{Ipac}{t^+} &= 0.0 \\
			...
			\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	IP & $\dfl{t^-}\svar{Isc}{t^-}(\svar{Ipac}{t^-} +$\par
		$\qquad\qquad \yfr{\svar{Sd}{t^-}}{t}\svar{Ipnr}{t^-}\svar{Nt}{t^-})$
		& {$\begin{aligned}
			\svar{Ipac}{t^+} &= 0.0 \\
			\svar{Fac}{t^+} &= \begin{cases} \svar{Fac}{t^-} + \yfr{\svar{Sd}{t^-}}{t}\svar{Nt}{t^-}\attr{FER} & \text{if} \quad \attr{FEB}=\text{'N'} \\
...
\begin{functions}{LAM}
	...
	\hline
	PR & $\dfl{t^-}\sgn\svar{Nsc}{t^-}\svar{Prnxt}{t^-}$
		& {$\begin{aligned}
...
\begin{functions}{LAX}
	...
	\hline
	PI & $\dfl{t^-}\sgn(-1)\svar{Nsc}{t^-}\svar{Prnxt}{t^-}$ & {$\begin{aligned}
			\svar{Nt}{t^+} &= \svar{Nt}{t^-}+\sgn\svar{Prnxt}{t^-} \\
			\svar{Ipac}{t^+} &= \svar{Ipac}{t^-} + \yfr{\svar{Sd}{t^-}}{t}\svar{Ipnr}{t^-}\svar{Ipcb}{t^-} \\
			\svar{Fac}{t^+} &= \begin{cases} \svar{Fac}{t^-} + \yfr{\svar{Sd}{t^-}}{t}\svar{Nt}{t^-}\attr{FER} & \text{if} \quad \attr{FEB}=\text{'N'} \\
					\frac{\yfr{t^{FP-}}{t}}{\yfr{t^{FP-}}{t^{FP+}}}\attr{FER} & \text{else} \end{cases} \\
...
\begin{functions}{NAM}
	...
	IED & \pof{IED}{PAM} & \stf{IED}{LAM} \\
	\hline
	PR & $\dfl{t^-}\svar{Nsc}{t^-}(\svar{Prnxt}{t^-}-\svar{Ipac}{t^-}-$\par $\qquad\qquad \yfr{\svar{Sd}{t^-}}{t}\svar{Ipnr}{t^-}\svar{Ipcb}{t^-})$
		& {$\begin{aligned}
...
\begin{functions}{CLM}
	...
	FP & \pof{FP}{PAM}
		& \stf{FP}{PAM} \\
	\hline
	IP & $\dfl{t^-}(\svar{Ipac}{t^-}+\yfr{\svar{Sd}{t^-}}{t}\svar{Ipnr}{t^-}\svar{Nt}{t^-})$
		& {$\begin{aligned}
				\svar{Ipac}{t^+} &= 0.0 \\
				\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
...
\begin{functions}{STK}
	...
	TD & $\dfl{t^-}\sgn \attr{PTD}$
		& $\svar{Sd}{t^+}=t$ \\
	\hline
	DV$^{(fix)}$ & $\dfl{t^-}\sgn\attr{DVNP}$ & $\svar{Sd}{t^+}=t$ \\
	\hline
...
\begin{functions}{FXOUT}
	...
	STD & $\dfl{t^-}\sgn (\attr{NT}-\obs{rf}{i}{\svar{Tmd}{t}}\attr{NT2})$\par
		...
		& $\svar{Sd}{t^+}=t$ \\
	\hline
	STD$^{(1)}$ & $\dfl{t^-}\sgn\attr{NT}$ & $\svar{Sd}{t^+}=t$ \\
	\hline
	STD$^{(2)}$ & $\dfl{t^-}\sgn (-1)\attr{NT2}$ & $\svar{Sd}{t^+}=t$ \\
	\hline
	CD & \pof{CD}{PAM}
		& \stf{CD}{STK} \\
\end{functions}
...
\begin{functions}{SWPPV}
	...
	TD & \pof{TD}{STK} & {$\begin{aligned}
				...
				\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	IP & $\dfl{t^-}\sgn (\svar{Ipac}{t^-}+\yfr{\svar{Sd}{t^-}}{t}(\attr{IPNR}-\svar{Ipnr}{t^-})\svar{Nt}{t^-})$ & {$\begin{aligned}
				\svar{Ipac}{t^+} &= 0.0 \\
				\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	IP$^{(fix})$ & $\dfl{t^-}\sgn (\svar{Ipac1}{t^-}+\yfr{\svar{Sd}{t^-}}{t}\attr{IPNR}\svar{Nt}{t^-})$ & {$\begin{aligned}
				\svar{Ipac1}{t^+} &= 0.0 \\
				\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	IP$^{(var)}$ & $\dfl{t^-}\sgn (\svar{Ipac2}{t^-}-\yfr{\svar{Sd}{t^-}}{t}\svar{Ipnr}{t^-}\svar{Nt}{t^-})$ & {$\begin{aligned}
				\svar{Ipac2}{t^+} &= 0.0 \\
				\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
...
\begin{states}{SWAPS}
	...
	\hline
  	$\svar{Ipac}{}$ & $\svar{Ipac}{t_0} = \cldsv{\attr{CTST}_{FirstLeg}^{Contract}}{t_0}{Nac}{\attr{CNTRL}=r^{(1)}} + \cldsv{\attr{CTST}_{SecondLeg}^{Contract}}{t_0}{Nac}{\attr{CNTRL}=r^{(2)}}$
			& with\par
			{$\begin{aligned}
				r^{(1)} &= \begin{cases} RPA & \text{if}\quad \attr{CNTRL}=RFL \\
					RPL & \text{else} \end{cases} \\
...
\begin{states}{CEG}
	...
	$\svar{Fac}{}$ & $\svar{Fac}{t_0} = \begin{cases} 0.0 & \text{if} \quad \attr{FER}=\undef \\
			...
			{$\begin{aligned}  n &= \begin{cases} \attr{NT} & \text{if}\quad \attr{NT}\neq\undef \\
								\sum_{i=1}^{\mid I\mid} \cldsv{I(i)}{\attr{CDD}}{Nvl}{x} & \text{else} \end{cases} \\
						t^{FP-} &= \sup t \in \vec{t}^{FP}\mid t<t_0 \\
...
$I=\attr{CTST}_{CoveredContract}^{Contract}$\\
	...
	$\svar{Pos}{}$ & $\svar{Pos}{t_0} = \begin{cases}
							...
							\attr{CECV}\sum_{i=1}^{\mid I\mid} n_i & \text{else} \end{cases}$ & with\par
			$n_i=\begin{cases} \cldsv{I(i)}{t_0}{Nvl}{x} & \text{if}\quad \attr{CEGE}=NO \\
					\cldsv{I(i)}{t_0}{Nvl}{x} & \text{else if}\quad \attr{CEGE}=NI \\
					\quad + \cldsv{I(i)}{t_0}{Nac}{x} & \\
...
\begin{functions}{CEG}
	AD & \pof{AD}{PAM} & {$\begin{aligned}
					...
					\svar{Sd}{t^+} &= t \end{aligned}$}\par
with\par
			{$\begin{aligned} n_i &= \begin{cases} \cldsv{I(i)}{t}{Nvl}{x} & \text{if}\quad \attr{CEGE}=NO \\
					\cldsv{I(i)}{t}{Nvl}{x} & \text{else if}\quad \attr{CEGE}=NI \\
					\quad + \cldsv{I(i)}{t}{Nac}{x} & \\
...
\begin{states}{CEC}
	...
	$\svar{Pos}{}$ & $\svar{Pos}{t_0} = \begin{cases}
							0.0 & \text{if}\quad t_0\geq\svar{Tmd}{t_0} \\
							\min\left(\sum_{i=1}^{\mid I\mid} v_i, \attr{CECV}\sum_{i=1}^{\mid I\mid} n_i\right) & \text{else} \end{cases}$ &  with\par
			$n_i=\begin{cases} \cldsv{I(i)}{t_0}{Nvl}{x} & \text{if}\quad \attr{CEGE}=NO \\
					\cldsv{I(i)}{t_0}{Nvl}{x} & \text{else if}\quad \attr{CEGE}=NI \\
					\quad + \cldsv{I(i)}{t_0}{Nac}{x} & \\
					\obs{rf}{\cldca{I(i)}{MOC}}{t_0} & \text{else} \end{cases}$ \par
...
\begin{functions}{CEC}
	AD & \pof{AD}{PAM} & {$\begin{aligned}
					\svar{Pos}{t^+} &= \begin{cases}
							0.0 & \text{if}\quad t\geq\svar{Tmd}{t_0} \\
							\min\left(\sum_{i=1}^{\mid I\mid} v_i, \attr{CECV}\sum_{i=1}^{\mid I\mid} n_i\right) & \text{else} \end{cases} \\
					\svar{Sd}{t^+} &= t \end{aligned}$}\par
 with\par
			$n_i=\begin{cases} \cldsv{I(i)}{t_0}{Nvl}{x} & \text{if}\quad \attr{CEGE}=NO \\
					\cldsv{I(i)}{t_0}{Nvl}{x} & \text{else if}\quad \attr{CEGE}=NI \\
