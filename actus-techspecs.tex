...
\begin{states}{MRGNG}
  ...
  \hline
  $\svar{Pos}{}$ & $\svar{Pos}{t_0} = \sum\cldev{\attr{CTST}_{Underlying}^{Contract}}{t_0}{}$ & \\
  \hline
  $\svar{Mrvm}{}$ & $\svar{Mrvm}{t_0} = \begin{cases}
					0 & \text{if} \quad\attr{MRVM}=\undef \\
					\attr{MRVM} & \text{else} \end{cases}$ & \\
	\hline
	$\svar{Prf}{}$ & & Same as PAM \\
	\hline
	$\svar{Led}{}$ & & Same as PAM \\
...
\begin{functions}{MRGNG}
	AD & \pof{AD}{PAM} & {$\begin{aligned}
          \svar{Pos}{t^+} &= \sum\cldev{\attr{CTST}_{Underlying}^{Contract}}{t}{}\\
          \svar{Mrvm}{t^+} &= \svar{Mrvm}{t^-}+\svar{Pos}{t^+} \\
					\svar{Led}{t^+} &= t \end{aligned}$} \\
	\hline
	IMP & $\attr{MRIM}$ & {$\begin{aligned}
          \svar{Pos}{t^+} &= \sum\cldev{\attr{CTST}_{Underlying}^{Contract}}{t}{}\\
          \svar{Mrvm}{t^+} &= \attr{MRIM} \\
          \svar{Led}{t^+} &= t \end{aligned}$} \\
	\hline
	MP & $x$ where\newline
        {$x= \begin{cases}
        ...
        with $S = \svar{Mrvm}{t^-} + \sum\cldev{\attr{CTST}_{Underlying}^{Contract}}{t}$
         & {$\begin{aligned}
          \svar{Pos}{t^+} &= \sum\cldev{\attr{CTST}_{Underlying}^{Contract}}{t}{}\\
          \svar{Mrvm}{t^+} &= \attr{MRIM} \\
					\svar{Led}{t^+} &= t \end{aligned}$} \\
