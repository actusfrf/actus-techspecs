...
\begin{table}[H]
	...
	\hline
	XD & Expiration Date & Scheduled expiration of e.g. an OPTNS or FUTUR contract & 19 \\
	\hline
	STD & Settlement Date & Date when payment for derivatives is settled & 20 \\
	\hline
	AD & Analysis Event & Retrieves current contract states without alter these & 21 \\
	\hline
	\end{tabular}
...
\begin{functions}{LAX}
	...
	RR & \pof{RR}{PAM}  
		& {$\begin{aligned} 
			...
			\svar{Led}{t^+} &= t \end{aligned}$}\par
		with\par
		$\Delta r = \min(\max(\obs{rf}{\attr{RRMO}}{t}\attr{RRMT}+\attr{ARRATE}_i - \svar{Nrt}{t^-},\attr{RRPF}),\attr{RRPC})$ \par
	where\par
	$i = \sup k\in\Nat \mid \attr{ARPRANX}_k\leq t$\\
	\hline
	RRF & \pof{RRF}{PAM} 
		& {$\begin{aligned} 
			\svar{Nac}{t^+} &= \svar{Nac}{t^-} + \yfr{\svar{Led}{t^-}}{t}\svar{Nrt}{t^-}\svar{Icb}{t^-} \\
